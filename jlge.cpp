...
char poi_file[MAX_STRING], net_poi[MAX_STRING], net_poi_time[MAX_STRING], net_poi_word[MAX_STRING];
char emb_poi[MAX_STRING], emb_time[MAX_STRING], emb_word[MAX_STRING];
char poi_file_u[MAX_STRING], net_poi_u[MAX_STRING], net_poi_time_u[MAX_STRING], net_poi_word_u[MAX_STRING];
char emb_poi_u[MAX_STRING], emb_time_u[MAX_STRING], emb_word_u[MAX_STRING];
...
int *neg_table_v, *neg_table_t;
//USER
int *neg_table_vu;

int num_vertices_poi = 0, num_vertices_v = 0, num_vertices_r = 0, num_vertices_t = 0;
long long total_samples = 100, current_sample_count = 0, num_edges_vv = 0, num_edges_vt = 0, num_edges_vw = 0;
//USER
int num_vertices_poiu = 0, num_vertices_vu = 0;
long long num_edges_vvu = 0, num_edges_vtu = 0, num_edges_vwu = 0;

real init_rho = 0.025, rho;
real **emb_vertex_v, **emb_vertex_t, *sigmoid_table;
//USER
real **emb_vertex_vu;

int *vv_edge_source_id, *vv_edge_target_id, *vt_edge_source_id, *vt_edge_target_id, *vw_edge_source_id, *vw_edge_target_id;
double *vv_edge_weight, *vt_edge_weight, *vw_edge_weight;
//USER
int *vvu_edge_source_id, *vvu_edge_target_id, *vtu_edge_source_id, *vtu_edge_target_id, *vwu_edge_source_id, *vwu_edge_target_id;
double *vvu_edge_weight, *vtu_edge_weight, *vwu_edge_weight;

// Parameters for edge sampling
long long *alias_vv, *alias_vt, *alias_vw;
double *prob_vv, *prob_vt, *prob_vw;
//USER
// Parameters for edge sampling USER
long long *alias_vvu, *alias_vtu, *alias_vwu;
double *prob_vvu, *prob_vtu, *prob_vwu;
	          ...
	          int *&edge_source_id, int *&edge_target_id, double *&edge_weight, std::vector <std::string> &vertex, std::vector <double> &vertexd, int hash_flag, int flag, int dxflag)
{
	...
	for (int k = 0; k != num_edges; k++)
	{
		...
		if (flag==1){
			...
			if(dxflag==1)vid = SearchHashTable(name_v1, 3);
			if (vid == -1) std::cout<<"Error: false point type on line "<<k+1<<" : "<<name_v1<<" flag="<<flag<<" dxflag ="<<dxflag<<std::endl;
...
void ReadUSERs(char *USERS_file){
	...
	{
		fscanf(fin, "%s", name);
		vid = SearchHashTable(name, 3);
		if (vid == -1) {
			vid = AddVertex(name, vertex_poiu_name, vertex_poiu_degree, num_vertices_poiu, 3);
		}
	}
	fclose(fin);
}
...
void ReadData(){
	...
	ReadFile(net_poi_u, num_edges_vvu, num_vertices_vu, vvu_edge_source_id, vvu_edge_target_id, vvu_edge_weight, vertex_vu_name, vertex_vu_degree, 3, 1, 1);
	std::cout<<"Graph User-User: "<<"\t"<<num_vertices_vu<<"x"<<num_vertices_vu;
	std::cout<<"\t"<<"Number of edges:"<<"\t";
	std::cout<<num_edges_vvu<<"\n";
	

	ReadFile(net_poi_time, num_edges_vt, num_vertices_t, vt_edge_source_id, vt_edge_target_id, vt_edge_weight, vertex_t_name, vertex_t_degree, 1, 0, 0);
	std::cout<<"Graph Loc-Time: "<<"\t"<<num_vertices_v<<"x"<<num_vertices_t;
	std::cout<<"\t"<<"Number of edges:"<<"\t";
	std::cout<<num_edges_vt<<"\n";

	ReadFile(net_poi_word, num_edges_vw, num_vertices_vu, vw_edge_source_id, vw_edge_target_id, vw_edge_weight, vertex_vu_name, vertex_vu_degree, 2, 0, 0);
	...
	ReadFile(net_poi_time_u, num_edges_vtu, num_vertices_t, vtu_edge_source_id, vtu_edge_target_id, vtu_edge_weight, vertex_tu_name, vertex_tu_degree, 4, 0, 1);
	std::cout<<"Graph User-Time: "<<"\t"<<num_vertices_vu<<"x"<<num_vertices_t;
	std::cout<<"\t"<<"Number of edges:"<<"\t";
	std::cout<<num_edges_vtu<<"\n";

	ReadFile(net_poi_word_u, num_edges_vwu, num_vertices_v, vwu_edge_source_id, vwu_edge_target_id, vwu_edge_weight, vertex_v_name, vertex_v_degree, 5, 0, 1);
...
void *TrainJLGEThread(void *id)
{
	...
	while (1)
	{
		...
		lu = u;
		if(a<=2)for (int c = 0; c != dim; c++) vec_error[c] = 0;
		if(a>2)for (int c = 0; c != dim; c++) vec_erroru[c] = 0;
		...
		for (int d = 0; d != num_negative + 1; d++)
		{
			...
			lv = target;
			if(a<=2)Update(emb_vertex_v[lu], emb_vertex_target[lv], vec_error, label);
			if(a>2)Update(emb_vertex_vu[lu], emb_vertex_target[lv], vec_erroru, label);
		}
		if(a<=2)for (int c = 0; c != dim; c++) emb_vertex_v[lu][c] += vec_error[c];
		if(a>2)for (int c = 0; c != dim; c++) emb_vertex_vu[lu][c] += vec_erroru[c];
		count++;
	}
	return NULL;
}
...
void TrainJLGE() {
	...
	clock_t start = clock();
	printf("--------------------------------\n");
	for (a = 0; a < num_threads; a++) pt[a] = boost::thread(TrainJLGEThread, (void *)a);
...
int main(int argc, char **argv) {
	...
	if (argc == 1) {
		...
		printf("\nExamples:\n");
		printf("./JLGE -size 200 -negative 5 -samples 100 -rho 0.025 -threads 20\n\n");
		return 0;
	}
